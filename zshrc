...
PROMPT_PART_ONE=$'%{\e[0;34m%}%B┌─[%b%{\e[0m%}%{\e[1;32m%}%n%{\e[1;30m%}@%{\e[0m%}%{\e[0;36m%}%m%{\e[0;34m%}%B]%b%{\e[0m%} - %b%{\e[0;34m%}%B[%b%{\e[1;37m%}%~%{\e[0;34m%}%B]%b%{\e[0m%} - %{\e[0;34m%}%B[%b%{\e[0;33m%}'%D{"%a %b %d, %I:%M:%S"}%b$'%{\e[0;34m%}%B]%b%{\e[0m%}
%{\e[0;34m%}%B└─%B['
PROMPT_PART_TWO=$'%B]>%{\e[0m%}%b '
...
function zle-line-init zle-keymap-select {
   ...
   PROMPT="${PROMPT_PART_ONE}${${KEYMAP/vicmd/${NORMAL_MODE_COLOR}N${END_COLOR}}/(main|viins)/${INSERT_MODE_COLOR}I${END_COLOR}}${PROMPT_PART_TWO}"
   zle reset-prompt
}
zle -N zle-line-init
